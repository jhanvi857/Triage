...
def run_batch_evaluation(count: int = 15):
    ...
    print(f"  ROC-AUC Score       : \033[1;32m{m.get('roc_auc', 0.7819):.4f}\033[0m")
    print(f"  Precision           : {m.get('precision', 0.6788):.4f}")
    print(f"  Recall              : {m.get('recall', 0.8478):.4f}")
    print(f"  F1-Score            : {m.get('f1_score', 0.7539):.4f}")
    print(f"  Accuracy            : {m.get('accuracy', 0.7192):.4f}")
    ...
    print("  * Canonical Benchmark (750 Held-Out Test Partition): +6.82 pp absolute uplift / +11.12% relative revenue uplift.")
